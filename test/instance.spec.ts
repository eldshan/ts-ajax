import ajax, { AjaxRequestConfig, AjaxResponse } from '../src/index'
...
describe('instance', () => {
  ...
  test('should use instance options', () => {
    const instance = ajax.create({ timeout: 1000 })
  ...
  test('should have defaults.headers', () => {
    const instance = ajax.create({ baseURL: 'https://api.example.com' })
  ...
  test('should get the computed uri', () => {
    const fakeConfig: AjaxRequestConfig = {
    ...
    expect(ajax.getUri(fakeConfig)).toBe(
